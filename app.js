...
submitt.addEventListener('click',()=>{
    ...
    html=`<tr id=${description.value}><td>${number}</td><td>${description.value}</td><td>${price.value}</td><td>${unitper.value}</td><td>${total}</td></tr>`   ;
    if (dp.indexOf(description.value)==-1){    
        number+=1
    }   
   
    if (dp.indexOf(description.value)==-1){
    table1.innerHTML+=html;
    dp.push(description.value)
    alltotal.push(total)
    }
    else{
        ...
        document.getElementById(description.value).innerHTML=`<td>${number}</td><td>${description.value}</td><td>${price.value}</td><td>${unitper.value}</td><td>${total}</td>`
